...
|      | `dewire` | `(bytes: &[u8]) -> Option<Tree>` | ワイヤフォーマットからTreeへ変換 | tree.rs |
...
|  | `write` | `(src: &[u8], out_path: &str) -> Result<(), String>` | YAML→.rs出力[precompile] | dsl.rs |
...
| ParseError | `fmt` | `&self, f: &mut fmt::Formatter<'_> -> fmt::Result` | ※エラーを返す | provided.rs |
